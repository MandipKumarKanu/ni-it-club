...
const Events = () => {
  ...
  useEffect(() => {
    ...
    fetchEvents();
  }, [category, urlSearchTerm, page, activeTab]);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 relative z-10">
          ...
          <div className="flex flex-col gap-8 mb-12">
            <div className="flex justify-center">
              <div className="bg-ni-black p-2 inline-flex gap-2 shadow-brutal transform -rotate-1">
                <button
                  onClick={() => handleTabChange("upcoming")}
                  className={`px-8 py-3 font-black uppercase text-xl transition-all relative group ${
                    activeTab === "upcoming"
                      ? "bg-ni-neon text-ni-black translate-x-1 translate-y-1"
                      : "text-ni-white hover:text-ni-neon"
                  }`}
                >
                  <span className="relative z-10">Upcoming</span>
                  {activeTab === "upcoming" && (
                    <div className="absolute inset-0 border-2 border-ni-black" />
                  )}
                </button>
                <div className="w-1 bg-ni-white/20 self-stretch my-2 rounded-full" />
                <button
                  onClick={() => handleTabChange("past")}
                  className={`px-8 py-3 font-black uppercase text-xl transition-all relative group ${
                    activeTab === "past"
                      ? "bg-ni-pink text-ni-white translate-x-1 translate-y-1"
                      : "text-ni-white hover:text-ni-pink"
                  }`}
                >
                  <span className="relative z-10">Past</span>
                  {activeTab === "past" && (
                    <div className="absolute inset-0 border-2 border-ni-white" />
                  )}
                </button>
              </div>
            </div>
              ...
              {(activeTab === "past"
                ...
                <div>
                  <div className="flex items-center gap-3 mb-8">
                    ...
                    <h2 className="text-3xl font-black uppercase">
                      {page === 1 && !urlSearchTerm && activeTab === "upcoming"
                        ? "More Events"
                        : "Events"}
                    </h2>
